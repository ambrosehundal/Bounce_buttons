...
int main(void) {
    ...
    while (1) {
        if (adc.event == TRUE) {
            //update OLED()
            percent = ((adc.value)*100) / 1023;
            sprintf(str, "Potentiometer value: %4d\n %3.0d%%", adc.value, percent);
            //Print string on the OLED
            OledDrawString(str);
            OledUpdate();
            adc.event = FALSE; //reset event flag
        }
    }
...
void __ISR(_ADC_VECTOR, IPL2AUTO) AdcHandler(void) {
    ...
    avg = (ADC1BUF0 + ADC1BUF1 + ADC1BUF2 + ADC1BUF3 + ADC1BUF4 + ADC1BUF5 +
            ADC1BUF6 + ADC1BUF7) / (8.0);

    //Check if avg has different value than previous adc value 
    //set event to TRUE and change adc value
    if (avg != adc.value) {
        adc.event = TRUE;
        adc.value = avg;
